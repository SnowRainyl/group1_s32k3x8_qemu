...
/* MANUAL p.163 register table + following registers tables
* for all registers, the bits are listed and an enable bitmask
* is provided + MASK & RESET (Default Value) values
* MASK (here, code) indicates a mask for which all previous 
* bitmasks are included
*/
...
#define SPI_CTL_MODE     (1 << 1) /* MASTER */
...
* data transfers. The SPI module counts SPI_SCLK by WCC
...
/*
* not implemented, here given value 0x10 == reset (Default Value)
* [31:13] not assignable
...
* equation: 2^(n+1). The SPI_SCLK is determined according
* to the following equation: SPI_CLK = AHB_CLK / 2^(n+1).
*
* [7:0] CDR2
...
*
* info to retrieve: AHB_CLK, SMC(?????)
* momentanearly suppose it's the clock received from
* the board, via bus system -> update: p.175. 
* table "SPI Module Clock Source and Frequency"
...
/*
* p.174 [23:0] WTC Write Transmit Counter
* In master mode, this field specifies the burst number that
* should be sent to TXFIFO before automatically sending
* dummy burst when SMC is 1. For saving bus bandwidth,
* the dummy burst (all zero bits or all one bits) is sent by SPI
...
/* here creates a mask to then use it for indicatng 
* both the size of the tx and rx fifo which I suppose 
* will be done by using the shift values below 
* indeed [0:6] RF_CNT for receiver fifo size and
* [22:16] TF_CNT for transmitter fifo size
* and the proposed position of shift is 16, feasible then
*/
...
/*
* some more general and useful info for code interpretation
* s: instance of (spi) module
* regs[REG_INDEX(SPI_CTL_REG)]: allows accessing SPI_CTL_REG register 
* | to only change the wanted bit
* & changes all other bits, as well as the needed one if other bits are zeroed
* ~ not
*/
